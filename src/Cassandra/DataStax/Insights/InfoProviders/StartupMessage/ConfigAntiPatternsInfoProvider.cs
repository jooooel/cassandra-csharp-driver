...
#pragma warning restore 618
        ...
        public Dictionary<string, string> GetInformation(IInternalSession session)
        {
            var antiPatterns = new Dictionary<string, string>();

            var resolvedContactPoints = session.Metadata.ResolvedContactPoints;
            ...
            var contactPointsHosts = session
                                     .AllHosts()
            ...
            var loadBalancingPolicy = session.Configuration.Policies.LoadBalancingPolicy;
            antiPatterns = ConfigAntiPatternsInfoProvider.AddAntiPatterns(loadBalancingPolicy, antiPatterns);

            var retryPolicy = session.Configuration.Policies.RetryPolicy;
