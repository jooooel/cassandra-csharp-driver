...
namespace Cassandra.Tests
{
    ...
    public class PoliciesUnitTests
    {
        ...
        public void RoundRobinIsCyclicTest()
        {
            ...
            policy.Initialize(sessionMock.Object);
        ...
        public void DcInferringPolicyInitializeInfersLocalDc(bool implicitContactPoint)
        {
            ...
            var sessionMock = CreateSessionMock(hostList, implicitContactPoint: implicitContactPoint);
            var policy = new DcInferringLoadBalancingPolicy();
            policy.Initialize(sessionMock);
        ...
        public void DCAwareRoundRobinPolicyNeverHitsRemote()
        {
            ...
            var sessionMock = CreateSessionMock(hostList);
        ...
        public void DCAwareRoundRobinInitializeUsesBuilderLocalDc()
        {
            ...
            var sessionMock = CreateSessionMock(hostList, "dc2");
        ...
        public void DCAwareRoundRobinInitializeDoesNotInferLocalDcWhenNotImplicitContactPoint()
        {
            ...
            var sessionMock = CreateSessionMock(hostList, implicitContactPoint: false);
            var policy = new DCAwareRoundRobinPolicy();
            var ex = Assert.Throws<InvalidOperationException>(() => policy.Initialize(sessionMock));
            Assert.AreEqual(
        ...
        public void DCAwareRoundRobinInitializeInfersLocalDcImplicitContactPoint()
        {
            ...
            var sessionMock = CreateSessionMock(hostList, implicitContactPoint: true);
            var policy = new DCAwareRoundRobinPolicy();
            policy.Initialize(sessionMock);
            Assert.AreEqual(HostDistance.Local, policy.Distance(hostList[0]));
            Assert.AreEqual(HostDistance.Remote, policy.Distance(hostList[1]));
        ...
        public void DCAwareRoundRobinInitializeNotMatchingDcThrows()
        {
            ...
            var ex = Assert.Throws<ArgumentException>(() => policy.Initialize(sessionMock));
            Assert.IsTrue(
        ...
        public void DCAwareRoundRobinInitializeNotMatchingDcFromBuilderLocalDcThrows()
        {
            ...
            var sessionMock = CreateSessionMock(hostList, "not_valid_dc");
            var policy = new DCAwareRoundRobinPolicy();
            var ex = Assert.Throws<ArgumentException>(() => policy.Initialize(sessionMock));
            Assert.IsTrue(
                ex.Message.Contains("Datacenter not_valid_dc does not match any of the nodes, available datacenters:"), 
        ...
        public void DCAwareRoundRobinPolicyWithNodesChanging()
        {
            ...
            //Initialize the balancing policy
            sessionMock.Configuration.LocalDatacenterProvider.Initialize(sessionMock);
            var policy = new DCAwareRoundRobinPolicy(localDc);
            policy.Initialize(sessionMock);
            ...
            actionList.Insert(200, () =>
            {
                ...
                Mock.Get(sessionMock).Raise(c => c.HostAdded += null, host);
                hostList.Add(host);
            });
            ...
            actionList.Insert(400, () =>
            {
                ...
                Mock.Get(sessionMock).Raise(c => c.HostRemoved += null, host);
            });
        ...
        private IInternalSession CreateSessionMock(
            ...
            bool implicitContactPoint = false)
        {
            ...
            Mock.Get(session).SetupGet(c => c.ImplicitContactPoint).Returns(implicitContactPoint);
            if (hostList != null)
            {
                var cc = Mock.Of<IControlConnection>();
                Mock.Get(cc).SetupGet(c => c.Host).Returns(hostList.First());
                Mock.Get(session).Setup(c => c.AllHosts()).Returns(hostList);
